...
for (i in 1:12){
  ...
  row <- data.frame(
    # Intercept
    int = altCoorModel$summary.fixed$mean[1],
    intU = altCoorModel$summary.fixed$`0.975quant`[1],
    intL = altCoorModel$summary.fixed$`0.025quant`[1],
    
    # Altitude 
    alt = altCoorModel$summary.fixed$mean[2],
    altU = altCoorModel$summary.fixed$`0.975quant`[2],
    altL = altCoorModel$summary.fixed$`0.025quant`[2],
    
    # Time linear effect 
    tml = altCoorModel$summary.fixed$mean[3],
    tmlU = altCoorModel$summary.fixed$`0.975quant`[3],
    tmlL = altCoorModel$summary.fixed$`0.025quant`[3],
    
    # Easting effect 
    eas = altCoorModel$summary.fixed$mean[4],
    easU = altCoorModel$summary.fixed$`0.975quant`[4],
    easL = altCoorModel$summary.fixed$`0.025quant`[4],
      
    # Northing effect 
    nor = altCoorModel$summary.fixed$mean[5],
    norU = altCoorModel$summary.fixed$`0.975quant`[5],
    norL = altCoorModel$summary.fixed$`0.025quant`[5],
    
    # Marginal Precision
    prcMarg = altCoorModel$summary.hyperpar$mean[1],
    prcMargU = altCoorModel$summary.hyperpar$`0.975quant`[1],
    prcMargL = altCoorModel$summary.hyperpar$`0.025quant`[1],
    
    # Precision for Time random effect
    prcTmr = altCoorModel$summary.hyperpar$mean[2],
    prcTmrU = altCoorModel$summary.hyperpar$`0.975quant`[2],
    prcTmrL = altCoorModel$summary.hyperpar$`0.025quant`[2],
    
    # Range for space
    rho = altCoorModel$summary.hyperpar$mean[3],
    rhoU = altCoorModel$summary.hyperpar$`0.975quant`[3],
    rhoL = altCoorModel$summary.hyperpar$`0.025quant`[3],
    
    # Stdev for space
    sig = altCoorModel$summary.hyperpar$mean[4],
    sigU = altCoorModel$summary.hyperpar$`0.975quant`[4],
    sigL = altCoorModel$summary.hyperpar$`0.025quant`[4],
  ...
  # Ridge plot storage
  intDists <- rbind(intDists, data.frame(INLA::inla.tmarginal(lnkFnc, altCoorModel$marginals.fixed[["Intercept"]]), month = i))
  altDists <- rbind(altDists, data.frame(INLA::inla.tmarginal(lnkFnc, altCoorModel$marginals.fixed[["alt"]]), month = i))
  tmlDists <- rbind(tmlDists, data.frame(INLA::inla.tmarginal(lnkFnc, altCoorModel$marginals.fixed[["time_lin"]]), month = i))
  easDists <- rbind(easDists, data.frame(INLA::inla.tmarginal(lnkFnc, altCoorModel$marginals.fixed[["ew"]]), month = i))
  norDists <- rbind(norDists, data.frame(INLA::inla.tmarginal(lnkFnc, altCoorModel$marginals.fixed[["ns"]]), month = i))
  rhoDists <- rbind(rhoDists, data.frame(INLA::inla.tmarginal(lnkFnc, altCoorModel$marginals.hyperpar[["Range for space"]]), month = i))
  
  # Store models
  modelList[[i]] <- altCoorModel
}
