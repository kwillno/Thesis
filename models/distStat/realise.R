...
for (i in 1:12){
  ...
  # Generate grid
  predictionGrid <- fm_cprod(
    coords,
    data.frame(
      year = 1981:2018, # Years to predict for
      STAID = 1         # Station ID to predict for (Needs to not be used in ECA)
    )
  )
  ...
  modelPrediction <- predict(
    ...
    ~ Intercept + alt + distSea + ew + ns + time_lin + time_re + station + space
  )
