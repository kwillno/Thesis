...
for (i in 1:12){
  ...
  row <- data.frame(
    ...
    # Precision for station error
    prcSta = model$summary.hyperpar$mean[3],
    prcStaU = model$summary.hyperpar$`0.975quant`[3],
    prcStaL = model$summary.hyperpar$`0.025quant`[3],
    
    # Range for space
    rho = model$summary.hyperpar$mean[4],
    rhoU = model$summary.hyperpar$`0.975quant`[4],
    rhoL = model$summary.hyperpar$`0.025quant`[4],
    
    # Stdev for space
    sig = model$summary.hyperpar$mean[5],
    sigU = model$summary.hyperpar$`0.975quant`[5],
    sigL = model$summary.hyperpar$`0.025quant`[5],
...
seaplot <- ggplot(df, aes(x = month, y = sea, ymin = seaL, ymax = seaU)) + 
  geom_point() +
  geom_errorbar() +
  ggtitle("Distance from sea effect")
...
staVarplot <- ggplot(df, aes(x = month, y = 1/prcSta, ymin = 1/prcStaU, ymax = 1/prcStaL)) + 
  geom_point() +
  geom_errorbar() +
  ggtitle("Station error Variance")
...
multiplot(margVarplot, tmrVarplot, staVarplot)
dev.off()
...
if (testing){
  # WAIC and DIC
  ggplot(scores, aes(x = month)) +
    geom_point(aes(y = DIC), color = "red") +
    geom_point(aes(y = WAIC), color = "blue") +
  ...
  # MLS and DSS
  ggplot(scores, aes(x = month)) +
    geom_point(aes(y = MLS), color = "red") +
    geom_point(aes(y = DSS), color = "blue") +
    labs(y = "Score")
  ...
  for (i in 1:nrow(scores)){
    row <- paste0(
      months[i], # Monthname
      " & ",
      round(scores[i,]$DIC, 4), # MLS
      " & ",
      round(scores[i,]$WAIC, 4), # WAIC
      " & ",
      round(scores[i,]$MLS, 6), # MLS
      " & ",
      round(scores[i,]$DSS, 6), # DSS
      "\\\\ "
    )
    lines <- c(lines, row)
  }
  
  fileConnection <- file(paste0("../../../data/objects/models/", modelName, "/scores.tex"))
